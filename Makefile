LANGUAGE_NAME := tree-sitter-penny
HOMEPAGE_URL := https://github.com/snotbane/tree-sitter-penny
VERSION := 0.1.0
DESCRIPTION := Penny grammar for tree-sitter
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/penny '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/penny
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/penny
